...
template<typename T>
BackFlipCtrl<T>::BackFlipCtrl(DataReader *data_reader, float _dt)
...
template<typename T>
void BackFlipCtrl<T>::OneStep(float _curr_time, bool b_preparation, LowlevelCmd *command)
{
	...
	for(int leg = 0; leg < 4; ++leg)
	{
		for(int jidx = 0; jidx < 3; ++jidx)
		{
			...
			{
				command->motorCmd[_leg_num].tau = DataCtrl::_jtorque[_leg_num];
				command->motorCmd[_leg_num].q = DataCtrl::_des_jpos[_leg_num] + 0 * _curr_time;
				command->motorCmd[_leg_num].dq = DataCtrl::_des_jvel[_leg_num];
			}
			else
			{
				command->motorCmd[_leg_num].tau = -DataCtrl::_jtorque[_leg_num];
			...
			command->motorCmd[_leg_num].mode = 10;
			command->motorCmd[_leg_num].Kp = DataCtrl::_Kp_joint[jidx];
			command->motorCmd[_leg_num].Kd = DataCtrl::_Kd_joint[jidx];
...
template
class BackFlipCtrl<double>;

template
class BackFlipCtrl<float>;
