...
/*
 * 逆归一化
 * value:归一化数，且属于[minLim,maxLim]区间(默认值分别为-1和1)
 * 返回值:value对应的真实值，属于[min, max]区间
 */
...
public:
	AvgCov(unsigned int size, std::string name, bool avgOnly = false, unsigned int showPeriod = 1000,
	       unsigned int waitCount = 5000, double zoomFactor = 10000)
			: _size(size), _showPeriod(showPeriod), _waitCount(waitCount), _zoomFactor(zoomFactor), _valueName(name),
			  _avgOnly(avgOnly)
	{
		_exp.resize(size);
	...
	void measure(VecX newValue)
	{
		...
		if(_measureCount > _waitCount)
		{
			...
			if(_measureCount % _showPeriod == 0)
			{
				...
				if(!_avgOnly)
				{
					std::cout << _zoomFactor << " Times Covariance of " << _valueName << std::endl << _zoomFactor * _cov
					          << std::endl;
				}
			}
		}
